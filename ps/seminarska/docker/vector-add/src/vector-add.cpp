...
// const int heightA = 9, widthA = 9, heightB = 9, widthB = 9;
...
void gpuMultiply(std::array<float, resultSize> &result,
  ...
  range<2> multiplyGlobalSize(WORKGROUP_SIZE, WORKGROUP_SIZE * groupCount);
  ...
  device_queue.submit([&](handler &cgh) {
    ...
    // stream for printing/debugging
    stream out(1024, 256, cgh);    

    cgh.parallel_for<class MatrixMultiplication>(
        nd_range<2>(multiplyGlobalSize, localSize), [=](nd_item<2> item) {
          ...
          for (int i = 0; i < Aiterations; i++) {
            ...
            for (int j = 0; j < Biterations; j++) {
              if (j * WORKGROUP_SIZE + lx < widthB && matrixB_y < heightB)
                localB[ly][lx] = b_in[matrixB_y * widthB + (j * WORKGROUP_SIZE + lx)];
              else
                localB[ly][lx] = 0;
              ...
              if (i * WORKGROUP_SIZE + ly < heightA && j * WORKGROUP_SIZE + lx < widthB)
                buf_out[matrixC_start + (i * WORKGROUP_SIZE + ly) * widthC + (j * WORKGROUP_SIZE + lx)] = sum;
  ...
  range<2> agregateGlobalSize(roundToNearest(heightA, WORKGROUP_SIZE), roundToNearest(widthB, WORKGROUP_SIZE));
  ...
  device_queue.submit([&](handler &cgh) {
    ...
    cgh.parallel_for<class MatrixAggregate>(
        nd_range<2>(agregateGlobalSize, localSize), [=](nd_item<2> item) {
          // stevilo niti po sirini
