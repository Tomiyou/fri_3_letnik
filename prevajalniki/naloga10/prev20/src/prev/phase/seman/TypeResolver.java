...
public class TypeResolver extends AstFullVisitor<Object, Integer> {
		...
		if (step == 4) {
			...
			while (pars.hasNext() && args.hasNext()) {
				...
				if (!parType.getClass().equals(argType.getClass()))
					throw Error("Call arguments' types need to match the declaration.", funDecl);
			}

			if (funDecl.pars().size() != callExpr.args().size())
				throw Error("Call arguments' count need to match the declaration.", funDecl);
		}
	...
	public Object visit(AstPfxExpr pfxExpr, Integer step) {
		...
		if (step == 5) {
			SemType exprTyp = SemAn.ofType.get(pfxExpr.expr()).actualType();
			SemType ret = null;
	...
	public Object visit(AstRecExpr recExpr, Integer step) {
		...
		if (step == 5) {
			SemType rec = SemAn.ofType.get(recExpr.rec()).actualType();
			...
			if (rec instanceof SemRecord) {
				for (AstCompDecl field : structs.get(rec).comps()) {
					if (field.name().equals(fieldName)) {
						...
						SemAn.bindCompDecl.put(recExpr, field);
	...
	public Object visit(AstWhereExpr whereExpr, Integer step) {
		...
		if (step == 5) {
			SemAn.ofType.put(whereExpr, SemAn.ofType.get(whereExpr.expr()).actualType() );
		}
	...
	public Object visit(AstAssignStmt assignStmt, Integer step) {
		...
		if (step == 5) {
			...
			} else if (!exprTyp1.getClass().equals(exprTyp2.getClass())) {
				throw Error("Assign statement types need to match.", assignStmt);
			} else {
