	...
	public static void main(String[] args) {
		try {
			...
			for (int argc = 0; argc < args.length; argc++) {
				if (args[argc].startsWith("--")) {
					...
					if (args[argc].matches("--num-regs=.*")) {
						registerCount = Integer.parseInt(args[argc].replaceFirst("^[^=]*=", ""));
						System.out.println("Register count: " + registerCount);
						continue;
					}
			...
			while (true) {
				...
				// Emite code.
				try (Emit emit = new Emit()) {
					emit.emit();
				}
				if (Compiler.cmdLineArgValue("--target-phase").equals("emit"))
					break;
