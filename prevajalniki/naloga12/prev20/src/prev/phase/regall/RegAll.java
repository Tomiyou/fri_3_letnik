					...
					for (int i = 0; i < code.instrs.size(); i++) {
						...
						if (tempReplaceIdx != -1) {
							...
							if (instr instanceof AsmOPER) {
								...
								if (offset > 255) {
									long _offset = offset - 255;
									AsmOPER setInstr = new AsmOPER("NEG `d0,0,255", null, setlDefs, null);
									code.instrs.add(i++, setInstr);

									while (_offset > 255) {
										AsmOPER subInstr = new AsmOPER("SUB `d0,`s0,255", setlDefs, setlDefs, null);
										code.instrs.add(i++, subInstr);
										_offset -= 255;
									}

									AsmOPER subInstr = new AsmOPER("SUB `d0,`s0," + _offset, setlDefs, setlDefs, null);
									code.instrs.add(i++, subInstr);
								} else {
									AsmOPER setInstr = new AsmOPER("NEG `d0,0," + offset, null, setlDefs, null);
									code.instrs.add(i++, setInstr);
								}
