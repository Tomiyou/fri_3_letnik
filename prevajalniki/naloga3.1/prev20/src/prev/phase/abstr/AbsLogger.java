...
public class AbsLogger implements AstVisitor<Object, String> {
	...
	@Override
	public Object visit(AstPtrExpr ptrExpr, String arg) {
		if (logger == null)
			return null;
		logger.begElement("node");
		logger.addAttribute("id", Integer.toString(ptrExpr.id()));
		logger.addAttribute("label", "PtrExpr");
		if (ptrExpr.ptr() != null)
			ptrExpr.ptr().accept(this, "");
		ptrExpr.location().log(logger);
		for (AstVisitor<Object, ?> subvisitor : subvisitors) {
			ptrExpr.accept(subvisitor, null);
		}
		logger.endElement();
		return null;
	}
